#!/usr/bin/env python3
# ~~~~~=======scp id_rsa.pub ubuntu@ip:~/.ssh/authorized_keys=======   HOW TO RUN   ==============~~~~~
...
class State_manager:
    def __init__(self, exchange):
        ...
        self.order_id_counter = (
            -1
        )  # start at -1 because we increment before returning the id
    ...
    def on_fill(self, message):
        ...
        if order_id in self.open_orders:
            ...
            self.positions_by_symbol[symbol] = (
                self.positions_by_symbol.get(symbol, 0) + size
            )
        else:
            print(
    ...
    def on_out(self, message):
        ...
        if order_id in self.open_orders:
            ...
            print(
                "Unexpectedly got out on order_id we did not expect to be live",
                order_id,
            )
    ...
    def open_and_pending_orders_in_symbol_and_direction_by_price_level(
        self, symbol, dir
    ):
        ...
        output = {}
        for order_id, order in chain(
            self.open_orders.items(), self.unacked_orders.items()
        ):
            if (
                order.symbol == symbol
                and order.dir == dir
                and order_id not in self.pending_cancels
            ):
    ...
    def set_orders_in_symbol_for_direction(self, symbol, dir, size_by_price_level):
        ...
        current_orders = (
            self.open_and_pending_orders_in_symbol_and_direction_by_price_level(
                symbol, dir
            )
        )
        for price_level in size_by_price_level | current_orders:
            ...
            if current_size_at_price_level == desired_size_for_price_level:
                ...
                self.send_order(
                    ...
                    dir,
                    price_level,
                    desired_size_for_price_level - current_size_at_price_level,
                )
            else:
                ...
                if desired_size_for_price_level != 0:
                    self.send_order(
                        symbol, dir, price_level, desired_size_for_price_level
                    )
